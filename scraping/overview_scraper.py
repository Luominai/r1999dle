...
class OverviewScraper(Scraper):
    ...
    def parse_page(self, page):
        ...
        for row in rows:
            ...
            overview = {}
            try:
                overview["name"] = name_cell.find_element(By.TAG_NAME, "a").text
            except:
            ...
            try:
                ...
                print(f"error getting image for {overview["name"]}")

            try:
                overview["page"] = str(name_cell.find_element(By.TAG_NAME, "a").get_attribute("href"))
                overview["lookup_name"] = unquote(overview["page"].split("/")[-1])
            except:
                print(f"error getting character page for {overview["name"]}")

            try:
                overview["rarity"] = int(rarity_cell.text)
            except:
                print(f"error getting rarity for {overview["name"]}")

            try:
                overview["afflatus"] = afflatus_cell.find_element(By.TAG_NAME, "a").text
            except:
                print(f"error getting afflatus for {overview["name"]}")

            try:
                overview["damage"] = damage_cell.text
            except:
                print(f"error getting damage type for {overview["name"]}")

            try:
                overview["tags"] = tags_cell.text.split("\n")
            except:
                print(f"error getting tags for {overview["name"]}")

            try:
                overview["birthday"] = birthday_cell.text
            except:
                print(f"error getting birthday for {overview["name"]}")

            try:
                overview["age"] = age_cell.text
            except:
                print(f"error getting age for {overview["name"]}")

            # print(overview)
            try:
                self.data[overview["lookup_name"]] = overview
            except:
                self.data[overview["name"]] = overview
        driver.quit()